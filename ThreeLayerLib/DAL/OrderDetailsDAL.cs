...
public class OrderDetailsDAL
{
    ...
    public bool InsertOrderDetails(List<OrderDetails> orderDetails)
    {
        try
        {
            ...
            {
                query = @"INSERT INTO OrderDetails(Order_ID, Product_Name, Product_Quantity) VALUES (@orderid, @bookname, @quantity);";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@orderid", ordDetail.OrderID);
                command.Parameters.AddWithValue("@bookname", ordDetail.BookName);
                command.Parameters.AddWithValue("@quantity", ordDetail.BookQuantity);
    ...
    public OrderDetails GetOrderDetails(MySqlDataReader reader)
    {
        ...
        odrDtls.BookName = reader.GetString("Product_Name");
        odrDtls.BookQuantity = reader.GetInt32("Product_Quantity");
        return odrDtls;
    }
}